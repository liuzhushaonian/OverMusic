...
public class LrcAdapter extends BaseAdapter<LrcAdapter.ViewHolder> {
    ...
    private int color= OverApplication.getContext().getResources().getColor(R.color.colorCyan);
    private int defaultColor=OverApplication.getContext().getResources().getColor(R.color.colorBlack);
    private int index=0;
    private int pre_index=0;
    private PlayingAlumFragment fragment;
    private int center=-1;
    private float textSize=OverApplication.getContext().getResources().getDimension(R.dimen.lrc_text_size);

    public LrcAdapter(PlayingAlumFragment fragment) {
...
//            Log.d("view--height-->>",holder.view.getHeight()+"");
    ...
    public void changeIndex(long time){
    ...
    private int calculation(){
        int half_screen_width=OverApplication.getContext().getResources().getDisplayMetrics().widthPixels/2;//获取屏幕宽度的一半
        int item_height=OverApplication.getContext().getResources().getDimensionPixelSize(R.dimen.bottom_padding);//获取item高度，8dp左右
